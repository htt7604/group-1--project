...
import { loginUser } from '../redux/authSlice'; // 1. Import action Thunk
...
const Login = () => {
    ...
    // 3. Lấy trạng thái (status, error, isAuthenticated) từ Redux store
    ...
    const onSubmit = (e) => {
        e.preventDefault();
        // 4. Gửi action `loginUser` đi, Redux sẽ lo việc gọi API
        dispatch(loginUser(formData));
    };

    // 5. Tự động chuyển trang KHI trạng thái isAuthenticated thay đổi
    useEffect(() => {
        if (isAuthenticated) {
