...
const Profile = () => {
    ...
    const [user, setUser] = useState(initialUser);
    const [name, setName] = useState(initialUser?.name || '');
    const [loading, setLoading] = useState(!initialUser); // Chỉ loading nếu không có user ban đầu
    const [error, setError] = useState('');
    ...
    if (loading) return <div>Đang tải...</div>;if (error) return <div style={{ color: 'red' }}>{error}</div>;
    if (!user) return <div>Không có thông tin người dùng. Vui lòng đăng nhập.</div>;
    ...
    return (
        <div>
            ...
            <div style={{ marginBottom: '20px', textAlign: 'center' }}>
                {/* ✅ CẢI TIẾN: Hiển thị ảnh placeholder nếu avatar là 'no-photo.jpg' */}
                <img 
